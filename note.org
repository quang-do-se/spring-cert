...
* Spring Web MVC
** Definition:
    - In Spring MVC, which of these is being described below?  Select the one that applies best. It is usually prepared inside controller methods, and it carries information that the view needs to create the view output.
